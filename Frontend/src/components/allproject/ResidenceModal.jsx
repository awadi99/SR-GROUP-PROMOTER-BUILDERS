...
export default function ResidenceModal({ isOpen, onClose, unit }) {
    // Initialize with the first image or a null-safe fallback
    const [mainImage, setMainImage] = useState(unit?.images?.[0]);

    // Sync state when the unit object changes
    useEffect(() => {
    ...
    // If no unit is provided, do not render
    if (!unit) return null;
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
                                ...
                                <div className="space-y-3">
                                    <motion.img 
                                        ...
                                        initial={{ opacity: 0 }} 
                                        animate={{ opacity: 1 }}
                                        src={mainImage || '/placeholder-unit.jpg'} 
                                        alt={unit.type}
                                ...
                                <div className="flex flex-col justify-start lg:justify-center">
                                    <h3 className="text-xl sm:text-3xl font-medium text-white mb-4 sm:mb-6">{unit.type || "Unit Details"}</h3>
