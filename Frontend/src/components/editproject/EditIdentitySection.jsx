...
export default function EditIdentitySection({ onNext }) {
    ...
    // 2. Initialize form with data from the store
    const { register, handleSubmit, reset } = useForm({
        defaultValues: sections.identity
    });

    // 3. Sync form if store data changes externally
    useEffect(() => {
        reset(sections.identity);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full">
            ...
            {/* Optional Image Preview (Fallback included) */}
            {sections?.identity?.imageUrl && (
                <div className="group relative w-full sm:w-32 h-48 sm:h-32 overflow-hidden border border-[#1a1a1a]">
                    <img 
                        src={sections.identity.imageUrl} 
            ...
            <div className="grid grid-cols-1 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <textarea 
                        ...
                        rows={5} 
                        className="w-full bg-[#0a0a0a] border border-[#1a1a1a] p-4 text-white focus:ring-1 focus:ring-[#B08B57] outline-none transition-all resize-y text-sm"
