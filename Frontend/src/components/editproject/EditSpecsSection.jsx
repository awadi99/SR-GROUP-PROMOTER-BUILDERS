...
export default function EditSpecsSection({ onNext, onPrev }) {
    ...
    const { register, handleSubmit, reset } = useForm({
        defaultValues: sections.specs || { towers: '', floors: '', architect: '', rera: '' }
    });

    // 3. Sync form if store data changes externally
    useEffect(() => {
        reset(sections.specs);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full">
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-4 border-t border-[#1a1a1a] pt-8">
                <Button 
                    type="button" 
                    onClick={onPrev} 
                    className="w-full sm:w-auto px-8 py-3 border border-[#1a1a1a] bg-transparent hover:bg-[#1a1a1a] text-white transition-all duration-300 uppercase tracking-widest text-[12px]"
                >
                    Back
                </Button>
                <Button 
